...
export default function CustomerInquiriesPage() {
  ...
  const fetchInquiries = async (p = 1, s = search, st = statusFilter, cat = categoryFilter) => {
    setLoading(true);
    try {
      const stQ = st ? `&status=${st}` : "";
      const catQ = cat ? `&category=${cat}` : "";
      const { data } = await api.get(`/api/customer-inquiries?page=${p}&limit=20${stQ}${catQ}`);
      const filtered = s
        ? data.data.filter((i) =>
            i.fullName.toLowerCase().includes(s.toLowerCase()) ||
            i.email.toLowerCase().includes(s.toLowerCase()) ||
            i.phone.includes(s) ||
            (i.companyName || "").toLowerCase().includes(s.toLowerCase())
          )
        : data.data;
  ...
  const handleStatus = async (id, status) => {
    try {
      const { data } = await api.put(`/api/customer-inquiries/${id}/status`, { status });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await api.delete(`/api/customer-inquiries/${id}`);
  ...
  return (
    <div style={{ fontFamily: "'MonaSans', Arial, sans-serif" }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-[#3b2f1e]">Customer Inquiries</h1>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={(e) => { setSearch(e.target.value); fetchInquiries(1, e.target.value, statusFilter, categoryFilter); }}
        ...
        <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); fetchInquiries(1, search, statusFilter, e.target.value); }}
                ...
                {inquiries.map((inq) => (
                  <tr key={inq._id} className="border-b border-[#f0ebe3] last:border-0 hover:bg-[#faf7f3] transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-[#3b2f1e] text-sm">{inq.fullName}</p>
                      <p className="text-[11px] text-[#9e8f7e] mt-0.5">{inq.email}</p>
                      <p className="text-[11px] text-[#9e8f7e]">{inq.phone}</p>
                    </td>
                    <td className="px-6 py-4 text-[#9e8f7e] text-xs hidden md:table-cell">{inq.companyName || "—"}</td>
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <span className={`text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full ${categoryColors[inq.productCategory]}`}>
                        {inq.productCategory}
                      </span>
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-[#FFFDF9] rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8e0d5] sticky top-0 bg-[#FFFDF9] z-10">
              ...
              <button onClick={() => setSelected(null)} className="text-[#9e8f7e] hover:text-[#3b2f1e]"><X size={18} /></button>
            </div>

            <div className="px-6 py-5 flex flex-col gap-5">
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Full Name", value: selected.fullName, full: true },
                  ...
                  <div key={label} className={`bg-[#f5f0ea] rounded-xl px-4 py-3 ${full ? "col-span-2" : ""}`}>
                    ...
                    <p className="text-sm text-[#3b2f1e] font-medium break-all">{value}</p>
                  </div>
                ))}
              </div>

              {(selected.productsSKUs || selected.orderQuantity || selected.additionalRequirements) && (
                <div className="flex flex-col gap-3">
                  {selected.productsSKUs && (
                    <div className="bg-[#f5f0ea] rounded-xl px-4 py-3">
                      <p className="text-[9px] font-bold tracking-widest text-[#9e8f7e] uppercase mb-1">Products / SKUs</p>
                      <p className="text-sm text-[#3b2f1e]">{selected.productsSKUs}</p>
                    </div>
                  )}
                  {selected.orderQuantity && (
                    <div className="bg-[#f5f0ea] rounded-xl px-4 py-3">
                      <p className="text-[9px] font-bold tracking-widest text-[#9e8f7e] uppercase mb-1">Order Quantity</p>
                      <p className="text-sm text-[#3b2f1e]">{selected.orderQuantity}</p>
                    </div>
                  )}
                  {selected.additionalRequirements && (
                    <div className="bg-[#f5f0ea] rounded-xl px-4 py-3">
                      <p className="text-[9px] font-bold tracking-widest text-[#9e8f7e] uppercase mb-1">Additional Requirements</p>
                      <p className="text-sm text-[#3b2f1e] leading-relaxed">{selected.additionalRequirements}</p>
                    </div>
                  )}
                </div>
              )}
